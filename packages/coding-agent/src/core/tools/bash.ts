...
export const bashToolSystemPromptContribution = {
	...
	guidelines: ["You can inspect TAU_* environment variables for current model and session details."],
} as const;
...
function resolveSpawnContext(
	...
	delete env.TAU_SESSION_ID;
	delete env.TAU_SESSION_FILE;
	delete env.TAU_PROVIDER;
	delete env.TAU_MODEL;
	delete env.TAU_REASONING_LEVEL;
	if (exposeSessionEnvironment && ctx) {
		const model = ctx.model;
		env.TAU_SESSION_ID = ctx.sessionManager.getSessionId();
		const sessionFile = ctx.sessionManager.getSessionFile();
		if (sessionFile) env.TAU_SESSION_FILE = sessionFile;
		if (model) {
			env.TAU_PROVIDER = model.provider;
			env.TAU_MODEL = model.id;
		}
		if (ctx.thinkingLevel) env.TAU_REASONING_LEVEL = ctx.thinkingLevel;
	}
...
export interface BashToolOptions {
	...
	shellPath?: string;
	/** Expose current Tau session metadata as TAU_* environment variables. Default: true */
	exposeSessionEnvironment?: boolean;
