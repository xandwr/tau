...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".tau", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".tau", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".tau", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".tau"), { recursive: true });
			writeFileSync(join(cwd, ".tau", "settings.json"), "{}");
