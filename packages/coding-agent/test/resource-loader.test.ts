...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".tau", "prompts");
			...
			const projectSkillDir = join(cwd, ".tau", "skills", "collision-skill");
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".tau", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".tau", "themes"), { recursive: true });
	...
	pi.registerCommand("shared", {
		...
		handler: async () => {},
			...
			symlinkSync(sharedExtDir, join(cwd, ".tau", "extensions"), "dir");
			...
			expect(extensionsResult.extensions[0].path).toBe(join(cwd, ".tau", "extensions", "shared.ts"));
		});

		it("should load user extensions before trust and reuse them after trust resolves", async () => {
			const userExtDir = join(agentDir, "extensions");
			const projectExtDir = join(cwd, ".tau", "extensions");
...
export default function(pi) {
	...
	pi.registerCommand("project-trusted", {
		...
		handler: async () => {},
			...
			const extensionsResult = loader.getExtensions();
			expect(extensionsResult.extensions.map((extension) => extension.path)).toEqual([
				join(cwd, ".tau", "extensions", "project.ts"),
				join(userExtDir, "user.ts"),
			]);
...
Content`,
		...
		it("should discover SYSTEM.md from cwd/.tau", async () => {
			const piDir = join(cwd, ".tau");
