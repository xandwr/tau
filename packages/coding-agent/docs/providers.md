...
Pi supports subscription-based providers via OAuth and API key providers via environment variables or auth file. Built-in catalogs ship with pi; configured providers may refresh newer catalogs and cache them in `~/.tau/agent/models-store.json` for offline use.
...
Use `/logout` to clear credentials. Tokens are stored in `~/.tau/agent/auth.json` and auto-refresh when expired. OpenRouter instead mints a user-controlled API key that does not expire automatically.
...
Store credentials in `~/.tau/agent/auth.json`:
...
```bash
tau --provider amazon-bedrock --model us.anthropic.claude-sonnet-4-20250514-v1:0
```
...
tau --provider amazon-bedrock --model arn:aws:bedrock:us-east-1:123456789012:application-inference-profile/abc123
```
...
tau --provider cloudflare-ai-gateway --model "claude-sonnet-4-5"
```
...
export CLOUDFLARE_API_KEY=...           # or use /login
export CLOUDFLARE_ACCOUNT_ID=...
tau --provider cloudflare-workers-ai --model "@cf/moonshotai/kimi-k2.6"
```
...
**Via extensions:** Providers that need custom API implementations or OAuth flows can register them through the extension API.
