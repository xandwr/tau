...
test_root="$(mktemp -d "$temp_parent/tau-test.XXXXXX")"
...
touch "$test_root/.tau-test-owned" "$test_root/npm-userconfig" "$test_root/npm-globalconfig"
...
cleanup() {
	...
	case "$test_root" in
		"$temp_parent"/tau-test.*)
			if [[ -d "$test_root" && ! -L "$test_root" && -f "$test_root/.tau-test-owned" ]]; then
				rm -rf -- "$test_root"
			else
